      ...
      <div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
        <div>
          ...
          <p class="page-subtitle text-muted mb-0">
            Live overview of subscriptions and media unlock earnings
          </p>
        </div>
            ...
            <table class="table align-middle mb-0">
              ...
              <tbody>
                ...
                <tr v-for="entry in entries" :key="`${entry.type}-${entry.id}`">
                  ...
                  <td>{{ entry.modelName || 'Unknown model' }}</td>
...
export default {
  ...
  watch: {
    ...
    'filters.modelId': 'handleFilterChange',
  ...
  methods: {
    async initialize() {
      try {
        await Promise.all([this.fetchSummary(), this.fetchModels(), this.fetchEntries()]);
        this.connectSocket();
      } finally {
    ...
    async fetchSummary() {
      ...
      try {
        ...
        const { year, months, totals, availableYears } = response.data || {};
    ...
    async fetchModels() {
      try {
        const response = await api.get('/admin/revenue/models');
        this.models = response.data?.models || [];
      } catch (error) {
        console.error('Error fetching models for filters:', error);
        this.models = [];
      }
    },
    async fetchEntries() {
      ...
      try {
        ...
        if (this.filters.modelId) {
          params.modelId = this.filters.modelId;
        }
        ...
        const response = await api.get('/admin/revenue/entries', { params });
